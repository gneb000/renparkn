...
/// Returns first number extracted from file name.
fn extract_numbering(file_name: &OsStr) -> f32 {
    let mut found_digit = false;
    let mut num_str = String::new();
    for c in file_name.to_string_lossy().chars() {
        if found_digit && c == '.' {
            if num_str.contains('.') {
                break;
            }
...
fn generate_new_name(file_path: &Path, root_path: &Path) -> Option<PathBuf> {
    let parent_name = file_path.parent()?.file_name()?.to_str()?;
    let num = extract_numbering(file_path.file_name().unwrap());
...
fn main() {
    ...
    let rename_pairs: HashMap<PathBuf, Option<PathBuf>> = list_files_recursively(dir_path)
        .iter()
        .map(|p| (p.clone(), generate_new_name(p, dir_path)))
        .collect();
